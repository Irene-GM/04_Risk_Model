...
def plot_compare_histograms(ax, nrow, samples_per_leaf, ytest, pred_sk, pred_rf):
    ...
    titles_list = [r"$\bf{POI}$", r"$\bf{NB}$", r"$\bf{ZIP}$", r"$\bf{ZINB}$", r"$\bf{RF-Can}$"]
    ...
    for j in range(0, 5):
        ...
        if j==4 and nrow==1:
            ax[nrow, j].legend(loc="upper right", ncol=3, bbox_to_anchor=(-1.5, -5), prop={'size': 24})
...
path_tmp = r"/home/irene/PycharmProjects/04_Risk_Model/data/skewed_leaves/to_keep_for_taylor_2nd/testing_SPL{0}_NESTI{1}.csv"
...
path_fig_out = r"/home/irene/Pictures/1003_Compare_Histograms_100-800SPL.png"
