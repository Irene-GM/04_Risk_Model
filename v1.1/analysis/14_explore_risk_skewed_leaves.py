...
path_out_err = r"/home/irene/PycharmProjects/04_Risk_Model/data/skewed_leaves/explore_trees/long_run_4mod_v1.0_error_metrics_NESTI_5.csv"
path_out_zer = r"/home/irene/PycharmProjects/04_Risk_Model/data/skewed_leaves/explore_trees/long_run_4mod_v1.0_proportion_zeros"
...
def save_tree_graph(ensemble, t):
    outfile = r"/home/irene/PycharmProjects/04_Risk_Model/data/skewed_leaves/explore_trees/trees_png/tree_plotted_{0}_{1}.png"
    for i in range(t):
        # print("I: ", i)
        # print("Len of estimators: ", len(ensemble.estimators_[i]))
        now = datetime.datetime.now()
        nowstr = str(now.hour).zfill(2) + str(now.minute).zfill(2)
        dotfile = StringIO()
        export_graphviz(ensemble.estimators_[i], out_file=dotfile)
        graph = pydotplus.graph_from_dot_data(dotfile.getvalue())
        graph.write_png(outfile.format(i, nowstr))
        time.sleep(2)
...
def test_varying_samples_per_node(meta_m, m):
    ...
    headers = "dbuiltup;dforest;drecreation;dbrr;dwrl;dwrn;dwrr;dcamping;dcaravan;dcross;dgolf;dheem;dhaven;dsafari;dwater;attr;dbath;lu;lc;maxmeanhaz;maxstdhaz".split(";")
