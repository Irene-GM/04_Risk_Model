...
path_out_err = r"/home/irene/PycharmProjects/04_Risk_Model/data/skewed_leaves/to_keep_for_taylor_2nd/long_run_4mod_v1.0_error_metrics_NESTI_5_remove_1708.csv"
path_out_zer = r"/home/irene/PycharmProjects/04_Risk_Model/data/skewed_leaves/to_keep_for_taylor_2nd/long_run_4mod_v1.0_proportion_zeros_remove_1708"
                ...
                if len(dictionary[key]) == 4:
                    if len(dic_test[key]) > 0:
                        ...
                        if model not in ["Singular", "Assert", "ValueError", "AllZeros"]:
                            ...
                            # Patch from 17/08/19: retrieve R2 to show that the count
                            # models work fine at the leaf nodes

                            r2_score_leaf = r2_score(yte, tiny_pred)
                            print("R2 score leaf:", r2_score_leaf)
...
def test_varying_samples_per_node(meta_m, m):
    ...
    # path_out = r"/home/irene/PycharmProjects/04_Risk_Model/data/skewed_leaves/to_keep_for_taylor_2nd/testing_SPL{0}_NESTI{1}_remove_1708.csv"

    path_out = r"D:/UTwente/PycharmProjects/04_Risk_Model/data/skewed_leaves/to_keep_for_taylor_2nd/testing_SPL{0}_NESTI{1}_remove_1708.csv"
    ...
    samples_per_leaf = [100, 200, 400, 600, 800, 1000, 1200]
    ...
    for spl in samples_per_leaf:
        ...
        for n_esti in n_estimators:
            ...
            stack = np.hstack((meta_m_test,  ytest.reshape(-1, 1), pred_sk, pred_rf.reshape(-1, 1)))
...
def plot_pred_vs_true(ax, nrow, samples_per_leaf, ytest, pred_sk, pred_rf):
    ...
    print(pred_sk.shape, pred_rf.shape)

    for j in range(0, 5):
        print(nrow, j)
        ax[nrow, j].set_facecolor("#EEEEEE")
        ax[nrow, j].grid()
        ax[nrow, j].plot(ytest, ytest, "-")

        if j == 4:
            ax[nrow, 4].plot(ytest, pred_rf, "o", markersize=4, color="black")
        else:
            ax[nrow, j].plot(ytest, pred_sk[:, j], "o", markersize=4, color="black")

        ax[nrow, j].set_title(titles_list[j] + same_part.format(samples_per_leaf), size=20)
        ax[nrow, j].set_xlabel("Predicted", size=16)
        ax[nrow, j].set_ylabel("True values", size=16)
