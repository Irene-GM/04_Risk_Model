...
extent_zoom = [4.4, 6.6, 51.85, 52.7]
...
gs = gridspec.GridSpec(3, 3, height_ratios=[10, 10, 2])
...
ax4 = plt.subplot(gs[1, 0], projection=ccrs.Mercator())
ax5 = plt.subplot(gs[1, 1], projection=ccrs.Mercator())
ax6 = plt.subplot(gs[1, 2:4], projection=ccrs.Mercator())
ax7 = plt.subplot(gs[2, :],  projection=ccrs.Mercator())

laxes = [ax1, ax2, ax3, ax4, ax5, ax6, ax7]
...
cax, kw = matplotlib.colorbar.make_axes(laxes[6], location='top', shrink=0.55, pad=0.05)
cbar = plt.colorbar(im, cax=cax, **kw, )
cbar.set_label("Tick bites", size=20, labelpad=-100)
cbar.ax.tick_params(labelsize=18)
cax.set_aspect('auto')
cax.yaxis.set_ticks_position('both')


laxes[6].set_visible(False)
...
pack = [loc_utr, loc_ams, loc_ape, loc_ams, loc_arn, loc_gou, loc_zwo, loc_rot]
...
for i in range(len(pack)):
    laxes[5].plot(lons[i], lats[i], color='black', linewidth=2, marker='s', transform=ccrs.Geodetic(), zorder=18)
...
# path_fig_out = r"C:\Users\irene\Pictures\paper4_fin\1303_SevenMaps_GnYlRd.png"
