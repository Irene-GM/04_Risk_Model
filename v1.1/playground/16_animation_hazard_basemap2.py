...
def update(i, m, xx, yy, data, sta_date):
    ...
    # plt.imshow(z.T, interpolation="None")
    # plt.show()

    im1 = m.pcolormesh(xx, yy, z.T, cmap=cmap, vmin=1, vmax=80, zorder=7)
...
def create_map(path_tif, sd, ed):
    ...
    m = Basemap(projection='merc', lon_0=3.0, lat_0=52.0, resolution='h',
                llcrnrlon=3.0, llcrnrlat=50.0, urcrnrlon=8.0, urcrnrlat=54.0)

    m = add_decorations(m)

    inproj, outproj = get_projections(m)
...
path_tif = r"G:/season_server/PycharmProjects/NL_predictors/data/versions/v9/maps_v9/2014/"

path_out_ani = r"C:/Users/irene/Videos/animationphd/ani.mp4"


sta_date = datetime.datetime(2014, 4, 15)
end_date = datetime.datetime(2014, 7, 15)
...
print("sta doy: ", sta_doy)
print("end doy: ", end_doy)
print("size of data: ", data.shape)
# chunk = data[sta_doy:end_doy, :, :]
...
ani = animation.FuncAnimation(plt.gcf(), update, frames=end_doy-sta_doy, fargs=(m, xx, yy, data, sta_date), interval=600)
