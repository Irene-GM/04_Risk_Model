...
def map_back_errors(meta, ytest, ypred, path_out):
    labels = []
    for i in range(ytest.shape[0]):
        if ytest[i] == 1 and ypred[i] == 1:
            labels.append("Ok_lo")
        elif ytest[i] == 2 and ypred[i] == 2:
            labels.append("Ok_hi")
        elif ytest[i] == 1 and ypred[i] == 2:
            labels.append("was_lo")
        else:
            labels.append("was_hi")

    stack = np.hstack((meta, np.array(labels).reshape(-1, 1)))
    np.savetxt(path_out, stack, fmt="%s", delimiter=";")
...
path_out_err = r"D:\UTwente\workspace\Special\04_Risk_Model\data\out\nl_classification_errors_RF_SPC2000.csv"
...
Yt = boxcox(Ynz, lmbda=-1.5)

print(np.unique(Yt, return_counts=True))

Ycl = classify_target(Yt)

print(np.unique(Ycl, return_counts=True))
...
plt.suptitle("Tick bite risk classification (positive samples -i.e. >0-, BOX-COX target)", size=24)
for samples_per_class in range(200, 2600, 200):
    ...
    # plt.colorbar()
    # plt.show()
    sb.heatmap(cm, annot=True, vmin=0, vmax=1200, cmap=plt.cm.Blues, annot_kws={"size": 20}, fmt='.0f' )
    i += 1
    print()

    if samples_per_class == 2000:
        map_back_errors(meta_te, ytest, ypred, path_out_err)
